...
.then(function(jsonData){
    ...
    jsonData.forEach(function(item,index){
        ...
        title.innerText=item.name.common;
        ...
        if(!("capital" in item))
            ...
            a=item.capital[0];
        cardText.innerHTML="<p>Capital: "+a+"<br>Region: "+item.region+"<br>Country Code: "+item.cca2+"</p>";
        ...
        bttn.onclick = function(){
            if(!("capital" in item)){
                modalTitle.innerText="No Data Available for this Country";
                bodyText.innerHTML="<p>So, Sorry!!!</p>";
                bodyText.style.fontFamily="Arial";
                bodyText.style.fontSize="28px";
            }else{
                fetch("https://api.openweathermap.org/data/2.5/weather?q="+a+"&appid=76e2ae3c30b69101e2f25e68c88f1e93")
                .then(function(data2){
                    return data2.json();
                })
                .then(function(fdata){
                    ...
                    modalTitle.innerText=item.name.common;
